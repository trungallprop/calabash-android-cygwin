class JavaKeystore
  ...
  def initialize(location, keystore_alias, password)
    ...
    log "Reading keystore data from keystore file '#{File.expand_path(location).gsub("/cygdrive/c", "C:")}'"

    keystore_data = system_with_stdout_on_success(Env.keytool_path, '-list', '-v', '-alias', keystore_alias, '-keystore', location.gsub("/cygdrive/c", "C:"), '-storepass', password, '-J"-Dfile.encoding=utf-8"')
  ...
  def sign_apk(apk_path, dest_path)
    ...
    unless system_with_stdout_on_success(Env.jarsigner_path, '-sigalg', 'MD5withRSA', '-digestalg', 'SHA1', '-signedjar', dest_path.gsub("/cygdrive/c", "C:"), '-storepass', password, '-keystore',  location.gsub("/cygdrive/c", "C:"), apk_path.gsub("/cygdrive/c", "C:"), keystore_alias)
