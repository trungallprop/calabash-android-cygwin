...
def main_activity(app)
  begin
    ...
    manifest_data = `"#{Env.tools_dir}/aapt" dump xmltree "#{app.gsub("/cygdrive/c", "C:")}" AndroidManifest.xml`
...
def aapt_dump(app, key)
  lines = `"#{Env.tools_dir}/aapt" dump badging "#{app.gsub("/cygdrive/c", "C:")}"`.lines.collect(&:strip)
...
def unsign_apk(path)
  files_to_remove = `"#{Env.tools_dir}/aapt" list "#{path.gsub("/cygdrive/c", "C:")}"`.lines.collect(&:strip).grep(/^META-INF\//)
  if files_to_remove.empty?
    log "App wasn't signed. Will not try to unsign it."
  else
    system("\"#{Env.tools_dir}/aapt\" remove \"#{path.gsub("/cygdrive/c", "C:")}\" #{files_to_remove.join(" ")}")
  end
end

def zipalign_apk(inpath, outpath)
  cmd = %Q("#{Env.zipalign_path}" -f 4 "#{inpath.gsub("/cygdrive/c", "C:")}" "#{outpath.gsub("/cygdrive/c", "C:")}")
...
def fingerprint_from_apk(app_path)
  ...
  Dir.mktmpdir do |tmp_dir|
    Dir.chdir(tmp_dir) do
      ...
      cmd = "#{Env.keytool_path} -v -printcert -J\"-Dfile.encoding=utf-8\" -file \"C:/cygwin#{rsa_files.first}\""
      log cmd
      fingerprints = `#{cmd}`
