  ...
  module Operations
    ...
    class Device
      ...
      def install_app(app_path)
        ...
        succeeded = `#{adb_command} shell pm list packages`.lines.map{|line| line.chomp.sub("package:", "").sub("\r", "")}.include?(pn)
      ...
      def uninstall_app(package_name)
        ...
        succeeded = !(`#{adb_command} shell pm list packages`.lines.map{|line| line.chomp.sub("package:", "").sub("\r", "")}.include?(package_name))
      ...
      def screenshot(options={:prefix => nil, :name => nil})
        ...
        if ENV["SCREENSHOT_VIA_USB"] == "false"
          ...
          screenshot_cmd = "java -jar #{File.join("C:/cygwin", File.dirname(__FILE__), 'lib', 'screenshotTaker.jar')} #{serial} \"#{path}\""
          log screenshot_cmd
